...
export class Controller {
  ...
  renderPoints() {
    ...
    //Listando todos os pontos
    document.querySelector('#btn-filtro-todos').addEventListener('click',()=>{
      this.clearMap();
      this.map.getPoints().then((result)=>{
        for(let i = 0; i < result['points'].length; i++){
          if(this.validatePolygon(result['points'][i])){
            this.drawPoint(result['points'][i]);         
          }
        };
      }).catch((error)=>{console.error(error)});
    });

    //Listando os pontos interiores
    document.querySelector('#btn-filtro-inside').addEventListener('click', ()=>{
      this.clearMap();
      this.map.getPoints().then((result)=>{
        for(let i = 0; i < result['points'].length; i++){
          if(this.map.contains(result['points'][i]) && this.validatePolygon(result['points'][i])){
            this.drawPoint(result['points'][i]);
          }
        };
      }).catch((error)=>{console.error(error)});
    ...
    //Listando os pontos exteriores
    document.querySelector('#btn-filtro-outside').addEventListener('click', ()=>{
      this.clearMap();
      this.map.getPoints().then((result)=>{
        for(let i = 0; i < result['points'].length; i++){
          if(!this.map.contains(result['points'][i]) && this.validatePolygon(result['points'][i])){
  ...
  //Valida o polígono, verificando a posição x e y
  validatePolygon(point)
  {
      if(point['x'] >=0 && point['x'] <= this.mapObject.getAttribute('width')
      && point['y'] >=0 && point['y'] <= this.mapObject.getAttribute('height')){
        return true;
      } else {
        return false;
      };
  ...
  //Limpar o mapa, removendo os circle
  clearMap(){
